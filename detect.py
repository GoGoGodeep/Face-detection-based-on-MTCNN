...
# P网络:
p_cls = 0.6 #原为0.6
p_nms = 0.1 #原为0.5 p网络在框人脸时， 精度低，会画很多框，给高了 会保留大量的框
# R网络：
r_cls = 0.6 #原为0.6
r_nms = 0.5 #原为0.5
# R网络：
...
class Detector:
    ...
    def __pnet_detect(self, image): # ★p网络全部是卷积，与输入图片大小无关，可输出任意形状图片
        ...
        img = image
        w, h = img.size
        min_side_len = min(w, h) # 获取图片的最小边长 图像金字塔缩放，得到循环的终止条件
        ...
        while min_side_len > 12: # 直到缩放到小于等于12时停止
            ...
            #N5HW
            # print("_cls",_cls.shape)         # [1, 1, 203, 245]:N1WH：分组卷积的特征图的通道和尺寸★
            # print("_offest", _offest.shape) # [1, 4, 203, 245]:N4WH
    ...
    def __rnet_detect(self, image, pnet_boxes):
        ...
        #R网络只需要检测一次
        _cls, _offset = self.rnet(img_dataset) # ★★将24*24的图片传入网络再进行一次筛选 N1 ,N4
